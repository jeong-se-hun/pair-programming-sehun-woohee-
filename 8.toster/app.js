(() => {
  ...
  const toaster = {
    set() {
      [...document.querySelectorAll('.toast')].forEach(($toast, i, arr) => {
        // prettier-ignore
        const location =(arr.length - 1 - i) * parseInt(getComputedStyle($toast).getPropertyValue('--toast-height'), 10);
  ...
  $body.addEventListener('click', e => {
    ...
    if (e.target.textContent === 'Warning')
      toaster.render({ type: 'warning', title: 'Check it out!', message: 'this is a warning alert' });
